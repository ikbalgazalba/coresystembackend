...
- **BR-BE07-05** `[RESOLVED v1.3.1]` scope final OQ-BE07-01 ✅: **Maker-checker WAJIB** untuk: `DEALER_BANK_REFERENCE` (payout `[LOCKED]`), `BLACKLIST_OVERRIDE` (regulated AML), `GL_TRANSACTION_TYPE_LINK` (`[LOCKED]` CoA), `GENERAL_PARAMETER`, `MENU.trans_type_id_prefix`, `TRANSACTION_TYPE`/`APPROVAL_HIERARCHY_LEVEL` (menentukan routing approval), field identitas legal `DEALER`, `NUMBER_FORMAT` utk `code_type` ber-konsumsi `[LOCKED]` (`CREDIT_ID`). **Checker = Kepala Cabang** untuk SEMUA resource (cabang-scope, konsisten D-10). Checker ≠ maker (self-approval blocked D-01 S11). Maker-checker di master = kontrol BARU; jangan diklaim paritas. `credit_id` minting tetap BE (ADR-08/OQ-GT-02 LOCKED — FE hanya minta/menampilkan).
