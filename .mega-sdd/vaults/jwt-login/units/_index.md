...
> Generated by `mega-sdd:generate-units` on 2026-07-22. **Reconciled v1.2** (2026-07-22, Mode D sync): P1 OQs AR-1/AR-2/AR-6/FL-1 RESOLVED → U-007, U-008 unblocked.

## Summary

- **Total units:** 9
- **All task_type: create** (greenfield skeleton — no existing code to verify/extend)
- **P1-blocker units** (halt execute-bolts if OQ unresolved): ~~U-007 (OQ-AR-1, AR-6, FL-1), U-008 (OQ-AR-2)~~ → **NONE (all P1 OQs RESOLVED v1.2)** — U-007/U-008 unblocked
- **Risk: high** — U-007, U-008 (external LDAP UCS + DB integration; secrets must externalize, not hardcode)
...
flowchart TD
    ...
    U001 --> U007[U-007 LdapUcsService<br/>✅P1 resolved]
    U002 --> U003[U-003 UserRepository]
    U004[U-004 DTOs]
    U005 --> U006[U-006 SecurityConfig]
    U001 --> U006
    U003 --> U008[U-008 AuthUserController + config<br/>✅P1 resolved]
    U004 --> U008
    U005 --> U008
    U007 --> U008
...
- **Wave 4:** U-008 (controller + config, needs U-003+U-004+U-005+U-007) — ✅ OQ-AR-2 RESOLVED v1.2 (newmojf existing DB)
...
| ID | Title | Complexity | Depends on | P1 blocker? |
|---|---|---|---|---|
| U-007 | Create LdapUcsService | medium | U-001 | ~~YES~~ → no (OQ-AR-1/6/FL-1 RESOLVED v1.2) |

### M-api
| ID | Title | Complexity | Depends on | P1 blocker? |
|---|---|---|---|---|
| U-004 | Create DTOs | small | — | no (OQ-AR-7 default verbatim) |
| U-008 | AuthUserController + config | medium | U-003, U-004, U-005, U-007 | ~~YES~~ → no (OQ-AR-2 RESOLVED v1.2) |
...
`/mega-sdd:execute-bolts --all` (runs waves in order; all P1 OQs RESOLVED v1.2 — no halt expected on U-007/U-008 from OQs; implementer uses verified LDAP UCS endpoints + newmojf DB, externalizes all secrets).
