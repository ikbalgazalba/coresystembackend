...
---<!-- BIND: confirmed=0 | conflict=0 | oq=14-claims/17-vault | state=NEW(greenfield) | advisor=clean(6cat)+3materialized -->
...
- Open Questions: **17** total — **P1: 4**, **P2: 6**, **P3: 7**
...
- **LDAP UCS** — layanan direktori autentikasi UCS yang dipakai newmojf (`LDAP_UCS_Utils.authLDAPNew`); `responseCode "00"/"01"` = sukses.
...
|---|---|---|---|---|
| OQ-AR-1 | LDAP UCS infra aktif untuk coresystembackend? | business / blocking | high | blocking — stakeholder/infra decision |
| OQ-AR-2 | Target DB: PostgreSQL newmojf existing atau DB baru? | business / blocking | high | blocking — keputusan infra |
...
| OQ-AR-6 | Prasyarat LDAP UCS (AES key, cred, URLs) tersedia? | business / blocking | high | blocking — infra/security provisioning |
...
| OQ-FL-1 | responseCode "00"/"01" LDAP UCS contract valid? | business / blocking | high | blocking — konfirmasi endpoint LDAP |
...
- [ ] **OQ-AR-1** [P1] [business] [conf: high]: coresystembackend belum punya infra LDAP UCS aktif — pakai LDAP UCS newmojf yang sama, atau perlu mock/fallback auth DB untuk dev? `[02-architecture.md]`
- [ ] **OQ-AR-2** [P1] [business] [conf: high]: target database — sambung ke PostgreSQL newmojf existing (10.95.1.43:5432/newmojf) atau DB baru khusus coresystembackend? Tabel users sudah berisi data user terdaftar? `[02-architecture.md]`
- [ ] **OQ-AR-6** [P1] [business] [conf: high]: prasyarat integrasi LDAP UCS — AES key, client credentials, partnerId/channelId, urlToken/urlVerifyPassword harus tersedia; dari mana? `[02-architecture.md]`
- [ ] **OQ-FL-1** [P1] [business] [conf: high]: kontrak response LDAP UCS (`responseCode`/`responseDescription`, kode sukses "00"/"01") — valid? Endpoint LDAP UCS apa yang dipakai? `[04-flows.md]`
