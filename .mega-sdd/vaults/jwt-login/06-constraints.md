...
- [x] **OQ-CN-1** [P3] [business] [conf: low]: regime compliance yang berlaku (PDP-Indonesia / lainnya) untuk penanganan kredensial login & data user? → **Resolved v1.3** (2026-07-23, PO/compliance decision): RESOLVED — **PDP Law Indonesia (UU No. 27/2022 Perlindungan Data Pribadi)** berlaku. Kredensial/password login & data user (uname, kodeMitra, urole) = data pribadi. Implikasi yang SUDAH terpenuhi di v1: (1) kredensial in-transit via HTTPS (LDAP UCS endpoint https, §B-006 trust store); (2) secret/credential di-externalize via env var, tidak hardcode (§D-002, OQ-AR-3); (3) error generic ke client, tidak bocor detail (§B-007, OQ-FL-3); (4) password tidak di-log (log hanya uname + responseCode generik); (5) `ddl-auto=none`, tidak menulis/mengubah data user. Implikasi YANG PERLU ditindaklanjuti compliance team: retensi data, hak subjek data (akses/hapus), DPIA formal, register pemrosesan — di luar scope teknis v1, koordinasi dengan tim compliance Bank Mega.
- [x] **OQ-CN-2** [P3] [tech / recommend] [conf: medium]: pakai Lombok (`@Slf4j` seperti newmojf) atau SLF4J manual `LoggerFactory`? → **Resolved v1.3** (2026-07-23, implementation-verified): VERIFIED — SLF4J manual `LoggerFactory.getLogger(...)` di semua class (JwtUtils, LdapUcsService, AuthUserController). Tidak ada dependency Lombok di `pom.xml` (zero-dependency logging). Memenuhi pack forbidden pattern (no `System.out`, §A-004). Jika future tim standar pakai Lombok, dapat migrasi ke `@Slf4j` (provided scope) tanpa breaking.
